...
//	  for 26.5 to update size
	  
	  public void AbreathFirstSearch(){
			AbreathFirstSearch((AVLTreeNode<E>) root);
		}

		protected void AbreathFirstSearch(AVLTreeNode<E> root){
			Queue<AVLTreeNode<E>> queue = new LinkedList<>();
			...
			while(!queue.isEmpty()){
				AVLTreeNode<E> temp = queue.poll();
				System.out.println(temp.element.toString() + " " + temp.size + " " + temp.height + ";");
				if(temp.left!= null)	queue.add((AVLTreeNode<E>) temp.left);
				if(temp.right!= null)	queue.add((AVLTreeNode<E>) temp.right);
			}
			
		}
	  
	  private void updateSize(E o){
		  java.util.ArrayList<TreeNode<E>> path = path(o);
...
//		  System.out.println(" k is " + k + " element is " + root.element.toString());
		  if(k<1 || k > root.size )	  return null;
		  if(k==1 && root.left==null)	return root.element;
		  else if (k==2 && root.left == null) return root.right.element;
		  else if(k <= ((AVLTreeNode) root.left).size) return find(k, (AVLTreeNode<E>) root.left);
		  else if(k == ((((AVLTreeNode) root.left).size) + 1)) return root.element;
		  else{
			  return find(k - ((AVLTreeNode<E>) root.left).size - 1, (AVLTreeNode<E>) root.right);
		  }
		}
		  
	  
	  public void printSize(){
		  printSize((AVLTreeNode) root);
	  }
	  
	  protected void printSize(AVLTreeNode<E> root){
		  if (root == null)return;
		  printSize((AVLTreeNode)root.left);
		  System.out.println("The node value is " + root.element.toString() + " size is " + root.size);
		  printSize((AVLTreeNode)root.right);
